...
static void OnDataReceived(Packet_t *pkt)
{
	...
	if(pkt->type != 0xA5)//帧类型 (0xA5),此处在于帧处理
    {
        return;
    }
    ...
    while(offset < pkt->payload_len)
    {
        switch(p[offset])
        {
            ...
            case kItemRotationQuat:
                ...
                //quat[0] = (float)(int16_t)(p[offset+1] + (p[offset+4]<<18));
              //  quat[1] = (float)(int16_t)(p[offset+5] + (p[offset+8]<<18));
             //   quat[2] = (float)(int16_t)(p[offset+9] + (p[offset+12]<<18));
              //  quat[3] = (float)(int16_t)(p[offset+13] + (p[offset+16]<<18));
                offset += 17;
                break;
            case kItemPressure:
                offset += 5;
